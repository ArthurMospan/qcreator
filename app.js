...
(function () {
  ...
  async function renderProject(id) {
    ...
    try {
      ...
      $('#pTitle').innerHTML = `${esc(project.name)} <span style="font-size:12px;color:var(--muted);font-weight:normal;cursor:pointer;background:#eee;padding:2px 6px;border-radius:4px" title="Клікни, щоб скопіювати ID" onclick="navigator.clipboard.writeText('${project.id}'); alert('ID скопійовано: ${project.id}')">ID: ${project.id}</span>`;
